    ...
    for (const item of importedData.validData) {
      try {
        if (item.purchaseDate) {
          ...
          if (dayjs(formattedDate).isValid()) item.purchaseDate = formattedDate
          else item.purchaseDate = null
        }
      ...
      <ReactSpreadsheetImport
        ...
        translations={{
          uploadStep: {
            ...
            manifestDescription:
              "(You will have a chance to rename or remove columns in next steps) Please note that entries with duplicate keys may not get uploaded. Likewise, columns need to be formatted correctly, (ex: Dates need to be in 'YYYY-MM-DD' format) else there maybe some loss of data"
          }
        }}
      />
