...
export default function LaptopSelectCell({ id, value, onChange, field, apiRef, softwareID, maxUsers }) {
  ...
  useEffect(() => {
    const fetchLaptopList = async () => {
      try {
        ...
        const filteredTemp = temp.filter((t) => !laptopAssetIDs.includes(t.laptopAssetID));
  ...
  return (
    <div>
      ...
      <Button variant='outlined' color='info' onClick={(event) => { setDeassignMode(false); handleOpenPopover(event); }}>Assign</Button>
      <Button variant='outlined' color='error' onClick={(event) => { setDeassignMode(true); handleOpenPopover(event); }}>Deassign</Button>
      <Popover
      ...
      >
        {deassignMode ?
              ...
              <List>
                {laptops.map((laptop) => (
                  <ListItem key={laptop.laptopAssetID}>
                    ...
                    <Button variant='outlined' color='error' onClick={() => { handleClosePopover(); handleDeassignLaptop(laptop.laptopAssetID) }}>
                      Deassign
                    </Button>
                  </ListItem>
                ))}
          ...
          (<Paper>
            <Button variant='outlined' color='info' sx={{ margin: 5 }} onClick={() => { handleClosePopover(); confirmLaptopAssignment() }}>Assign Selected</Button>
            <List>
