...
export default function CreateUserForm () {
  ...
  const handleSubmit = async e => {
    ...
    if (userRole == 'Admin' && (email && password && user_metadata) != null) {
      ...
      alert.show(
        'Some fields may be missing.Please note that only admins can create users'
      )
      ...
      <Paper elevation={3} className='paperStyle2'>
        ...
        <form className='div-centerstyle' noValidate autoComplete='off'>
          ...
          <FormControl required fullWidth style={{ margin: '20px auto' }}>
            ...
            >
              <MenuItem key={'Standard user'} value={'Standard'}>
                Standard User
              </MenuItem>
              <MenuItem key={'Administrator'} value={'Admin'}>
                Administrator
              </MenuItem>
            </Select>
          </FormControl>
